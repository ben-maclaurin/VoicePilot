defmodule VoicepilotWeb.CoreComponents do
  @moduledoc """
          ...
          <div class="">
            ...
            >
              ...
              <div id={"#{@id}-content"}>
                ...
                <div :if={@confirm != [] or @cancel != []} class="">
                  <.button
                    :for={confirm <- @confirm}
                    id={"#{@id}-confirm"}
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="">
        <.icon :if={@kind == :info} name="hero-information-circle-mini" class="" />
        <.icon :if={@kind == :error} name="hero-exclamation-circle-mini" class="" />
        <%= @title %>
      </p>
      <p class=""><%= msg %></p>
  ...
  def flash_group(assigns) do
    ...
    >
      Attempting to reconnect <.icon name="hero-arrow-path" class="animate-spin" />
    </.flash>
    """
  end
  ...
  def input(assigns) do
    ~H"""
    <div phx-feedback-for={@name} class="w-full flex flex-col items-start">
      <.label for={@id}><%= @label %></.label>
      <input
        type={@type}
        name={@name}
  ...
  def error(assigns) do
    ~H"""
    <p class="">
      <.icon name="hero-exclamation-circle-mini" class="" />
      <%= render_slot(@inner_block) %>
    </p>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(VoicepilotWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(VoicepilotWeb.Gettext, "errors", msg, opts)
    end
  end
